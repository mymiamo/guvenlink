...
final class UsomImporter
{
    ...
    public function import(): array
    {
        ...
        try {
            ...
            $this->runs->finish($runId, 'success', $stats, 'USOM canli API kullaniliyor; SQL import yapilmadi.');
    ...
    private function fetchPage(int $page, int $pageSize): array
    {
        ...
        if ($json === false) {
            throw new \RuntimeException('USOM verisi alinamadi: ' . $url);
        }

        $decoded = json_decode($json, true);
        if (!is_array($decoded) || !isset($decoded['models'])) {
            throw new \RuntimeException('USOM veri bicimi beklenen formatta degil.');
        }
