...
final class ThreatEntryRepository
{
    ...
    public function findActiveMatch(string $normalizedUrl, string $hostname): array
    {
        ...
        $stmt = $this->db()->prepare('
            ...
            ORDER BY FIELD(status, "white", "black", "suspicious"), FIELD(source, "manual", "usom"), updated_at DESC
        ');
    ...
    public function feedPage(int $page, int $perPage): array
    {
        ...
        $stmt = $this->db()->prepare('
            SELECT id, type, normalized_value AS value, status, source, reason, updated_at, is_active
            FROM threat_entries
            WHERE is_active = 1
            ORDER BY id ASC
    ...
    public function changesSince(string $since): array
    {
        $stmt = $this->db()->prepare('
            SELECT id, type, normalized_value AS value, status, source, reason, updated_at, is_active
            FROM threat_entries
    ...
    {
        $value = $this->db()->query('SELECT MAX(updated_at) FROM threat_entries')->fetchColumn();
    ...
    public function upsertImportedEntry(array $data): string
    {
        ...
        $this->update((int) $existing['id'], $data + ['first_seen_at' => $existing['first_seen_at']]);
        return 'updated';
    }

    public function upsertManualEntry(array $data): int
    {
        $existing = $this->findBySourceAndValue('manual', $data['type'], $data['normalized_value'], $data['normalized_hash']);
        if ($existing === null) {
